...
const contentRule = <T extends z.ZodRawShape>(shape: T) => z.object(shape).strict().refine(
  (value) => {
    ...
    return title === undefined || body === undefined || title.trim().length > 0 || body.trim().length > 0;
  },
