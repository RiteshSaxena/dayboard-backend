...
export class TurnstileService {
  ...
  async verify(token: string, expectedAction: string): Promise<void> {
    const hostnames = new Set(
      this.runtime.env.TURNSTILE_HOSTNAMES.split(",").map((value) => value.trim()).filter(Boolean),
    );
    if (!token || token.length > 2048 || hostnames.size === 0) throw forbidden("Bot verification failed");
