...
export class MailService {
  ...
  sendVerification(input: { name: string; email: string; token: string }): void {
    this.schedule(
      renderEmail(input.email, {
        ...
        paragraphs: [`${input.name}, confirm that ${input.email} is yours and your board is ready to sync.`],
        ctaLabel: "Confirm email",
        ctaUrl: `${this.runtime.env.APP_URL}/verify/${encodeURIComponent(input.token)}`,
        footer: "This link works for 24 hours. If you did not create a Dayboard account, you can ignore this email.",
      }),
    );
  }
  ...
  sendReset(input: { email: string; token: string }): void {
    this.schedule(
      renderEmail(input.email, {
        ...
        footer: "This link works for one hour and can be used once. If you did not ask for this, your password is unchanged and you can ignore this email.",
      }),
    );
  }
  ...
  sendInvite(input: { email: string; inviter: string; inviterEmail: string; org: string; role: string; token: string }): void {
    const article = input.role === "admin" ? "an admin" : input.role === "guest" ? "a guest" : "a member";
    this.schedule(
      renderEmail(input.email, {
        ...
        paragraphs: [`${input.inviter} (${input.inviterEmail}) added you as ${article} of ${input.org}. Accept to see the team's projects, tasks, and notes.`],
        ctaLabel: `Join ${input.org}`,
        ctaUrl: `${this.runtime.env.APP_URL}/invite/${encodeURIComponent(input.token)}`,
        footer: "The invitation expires in 7 days. If you were not expecting it, you can ignore this email.",
      }),
    );
  }

  sendMemberJoined(input: { to: string; name: string; email: string; org: string; role: string; orgId: string }): void {
    const article = input.role === "admin" ? "an admin" : input.role === "guest" ? "a guest" : "a member";
    this.schedule(
      renderEmail(input.to, {
        ...
        paragraphs: [`${input.name} (${input.email}) accepted your invitation and is now ${article}.`],
