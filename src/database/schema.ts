...
export const tasks = sqliteTable(
  ...
  (table) => [
    ...
    check("tasks_due_date_check", sql`${table.dueDate} is null or ${table.dueDate} glob '????-??-??'`),
  ],
);
...
export const activity = sqliteTable(
  "activity",
  {
    ...
    orgId: text("org_id", { length: 21 })
      .notNull()
      .references(() => orgs.id, { onDelete: "cascade" }),
    projectId: text("project_id", { length: 21 }).references(() => projects.id, { onDelete: "cascade" }),
    taskId: text("task_id", { length: 21 }).references(() => tasks.id, { onDelete: "set null" }),
