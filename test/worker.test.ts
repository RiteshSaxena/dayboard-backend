...
import { MAIL_SERVICE } from "../src/modules/mail/mail.tokens";
import { MailService } from "../src/modules/mail/mail.service";
import { RATE_LIMIT_SERVICE, TURNSTILE_SERVICE } from "../src/modules/security/security.tokens";
...
import { RUNTIME_CONTEXT } from "../src/core/di/tokens";
...
async function seedActor(): Promise<void> {
  ...
  await env.DB.batch([
    env.DB.prepare("INSERT INTO users (id,email,email_verified_at,name,password_hash,created_at,updated_at) VALUES (?,?,?,?,?,?,?)")
      .bind(USER_ID, "test@dayboard.space", timestamp, "Test User", "unused", timestamp, timestamp),
    env.DB.prepare("INSERT INTO orgs (id,name,slug,personal,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?)")
      .bind(ORG_ID, "Test Board", "test-board", 1, USER_ID, timestamp, timestamp),
    env.DB.prepare("INSERT INTO memberships (org_id,user_id,role,joined_at) VALUES (?,?,?,?)")
      .bind(ORG_ID, USER_ID, "owner", timestamp),
    env.DB.prepare("INSERT INTO sessions (id,user_id,token_hash,created_at,last_seen_at,expires_at) VALUES (?,?,?,?,?,?)")
      .bind("ses000000000000000000", USER_ID, await sha256(TOKEN), timestamp, timestamp, timestamp + 86_400_000),
  ]);
}
...
describe("Dayboard Worker", () => {
  ...
  it("signs up with a bearer session and personal organization", async ({ expect }) => {
    ...
    application.providers
      .register(MAIL_SERVICE, (c) => new TestMailService(c.resolve(RUNTIME_CONTEXT)))
      .register(RATE_LIMIT_SERVICE, (c) => new TestRateLimitService(c.resolve(RUNTIME_CONTEXT)))
      .register(TURNSTILE_SERVICE, (c) => new TestTurnstileService(c.resolve(RUNTIME_CONTEXT)));
    const ctx = createExecutionContext();
    const signupResponse = await application.app.request("https://api.dayboard.space/api/auth/signup", {
      method: "POST",
    ...
    const signup = await signupResponse.json<{ data: { token: string; personalOrgId: string } }>();
    ...
    const me = await meResponse.json<{ data: { user: { email: string }; orgs: unknown[] } }>();
  ...
  it("creates and reads board data through the modular API", async ({ expect }) => {
    ...
    const taskResponse = await request(`/api/projects/${projectBody.data.id}/tasks`, {
      method: "POST",
      headers,
      body: JSON.stringify({ title: "Ship API", description: "Test it", status: "doing" }),
    });
    ...
    const boardResponse = await request(`/api/orgs/${ORG_ID}/board`, { headers });
    expect(boardResponse.status).toBe(200);
    const board = await boardResponse.json<{ data: { projects: unknown[]; tasks: unknown[]; notes: unknown[] } }>();
